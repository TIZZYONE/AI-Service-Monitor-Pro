...
$StartScript = Join-Path $ProjectDir "scripts\prod\start_backend.bat"
if (-not (Test-Path $StartScript)) {
    Write-Host "错误: 未找到启动脚本: $StartScript" -ForegroundColor Red
    pause
    exit 1
}
...
# 任务名称
$TaskName = "AI Service Monitor Pro"
...
# 创建任务操作
$Action = New-ScheduledTaskAction -Execute "cmd.exe" -Argument "/c `"$StartScript`"" -WorkingDirectory $BackendDir
...
# 创建任务描述
$Description = "AI Service Monitor Pro - 任务管理系统后端服务"
...
if ($response -eq "y" -or $response -eq "Y") {
    ...
    if ($taskInfo.LastTaskResult -eq 0) {
        ...
        Write-Host "服务可能启动失败，请检查任务计划程序" -ForegroundColor Yellow
    }
}
...
Write-Host "  启动服务: Start-ScheduledTask -TaskName '$TaskName'" -ForegroundColor Green
Write-Host "  停止服务: Stop-ScheduledTask -TaskName '$TaskName'" -ForegroundColor Green
