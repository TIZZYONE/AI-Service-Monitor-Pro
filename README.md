...
### 后端功能
- ✅ **任务管理**: 创建、查询、更新、删除任务
- ✅ **任务调度**: 支持一次性、每日、每周、每月重复执行，支持设置结束时间自动停止
- ✅ **任务执行**: 自动执行激活命令和主程序命令
...
**一次性任务（NONE）**
- 在开始时间执行一次
- 如果设置了结束时间，会在结束时间停止正在运行的任务

**每日重复（DAILY）**
- 每天在开始时间的时、分、秒执行
- 如果设置了结束时间：
  - **开始和结束时间在同一天**：每天在结束时间自动停止，无限期重复执行
  - **开始和结束时间在不同天**：从开始日期到结束日期，每天在开始时间执行，在结束时间停止

**每周重复（WEEKLY）**
- 每周在开始时间的星期几和时、分、秒执行
- 如果设置了结束时间：
  - **开始和结束时间在同一天**：每周在结束时间的星期几和时、分、秒自动停止，无限期重复执行
  - **开始和结束时间在不同天**：从开始日期到结束日期，每周在开始时间的星期几执行，在结束时间的星期几停止

**每月重复（MONTHLY）**
- 每月在开始时间的日期和时、分、秒执行
- 如果设置了结束时间：
  - **开始和结束时间在同一天**：每月在结束时间的日期和时、分、秒自动停止，无限期重复执行
  - **开始和结束时间在不同天**：从开始日期到结束日期，每月在开始时间的日期执行，在结束时间的日期停止

#### 示例

**示例1：每天执行1小时**
- 重复类型：每日
- 开始时间：`2024-01-01 14:30:00`
- 结束时间：`2024-01-01 15:30:00`
- 执行逻辑：每天 14:30 开始执行，15:30 自动停止，无限期重复

**示例2：每周执行，有结束日期**
- 重复类型：每周
- 开始时间：`2024-01-01 14:30:00`（周一）
- 结束时间：`2024-01-10 15:30:00`（周三）
- 执行逻辑：每周一 14:30 执行，每周一 15:30 停止，1月10日之后不再执行
...
- 重复任务在服务重启后会自动重新加载并继续调度
- 如果设置了结束时间，系统会在指定时间自动停止任务
- 支持手动启动和停止任务
- 任务状态实时更新
- 超过结束日期的重复任务会自动从调度器中移除
